...
async function start() {
    try {
        console.log("🤖 Starting OGBEFIA Bot...\n");
        ...
        // MAIN MESSAGE HANDLER - FIXED FOR EVERYONE
        w.ev.on("messages.upsert", async ({ messages }) => {
            try {
                ...
                // REMOVED: if (a.key.fromMe) return - This was blocking commands from yourself!
                // Now everyone including yourself can use commands
                ...
                } else if (a.message.buttonsResponseMessage?.selectedButtonId) {
                    t = a.message.buttonsResponseMessage.selectedButtonId;
                } else if (a.message.listResponseMessage?.singleSelectReply?.selectedRowId) {
                    t = a.message.listResponseMessage.singleSelectReply.selectedRowId;
                } else {
                    return; // Not a text message
                }

                // Log EVERY message (including from yourself)
                console.log(`📩 ${g ? "Group" : "Private"} from ${a.key.participant || j}: ${t}`);

                // Auto reaction (works for everyone)
                ...
                // COMMANDS - NOW WORKS FOR EVERYONE INCLUDING YOURSELF
                if (t.startsWith(".")) {
                    const c = t.slice(1).split(" ")[0].toLowerCase();
                    const args = t.slice(1 + c.length).trim().split(" ");
                    console.log(`⚡ Command: ${c} from ${a.key.participant || j}`);
