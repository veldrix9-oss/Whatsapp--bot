...
async function startBot(n){
 try{
  ...
  if(sessions.has(n)){
   console.log(`ℹ️ ${n} already has an active session`);
   return;
  }
  ...
  if(state.creds.registered){
   ...
   activeUsers.set(n,{status:'connected',connectedAt:new Date().toISOString(),offlineMode:false});
   return;
  }
  ...
  const sock=makeWASocket({
   ...
   connectTimeoutMs:120000,emitOwnEvents:true
  });
  ...
  sock.ev.on("connection.update",async u=>{
   ...
   if(qr){
    ...
    // If QR appears, try pairing code again
    if(!pairingCodeSent){
     try{
...
╚══════════════════════════════════════╝`
   ...
   if(connection==="close"){
    ...
    if(c===401){
     console.log(`📱 ${n} - Waiting for pairing...`);
     // Don't delete from activeUsers if waiting for pairing
     if(!connectionEstablished){
      console.log(`🔄 Still waiting for ${n} to enter pairing code...`);
     }else{
      activeUsers.delete(n);
      sessions.delete(n);
     }
    }else{
     activeUsers.delete(n);
     sessions.delete(n);
     console.log(`♻ Reconnecting ${n} in 5 seconds...`);
...
╚══════════════════════════════════════╝`
  ...
  // Store session only after connection
  if(!sessions.has(n)){
   sessions.set(n,sock);
  }
 ...
 rl.question("\n👉 Choose (1-6): ",async c=>{
  if(c==='1'){
   rl.question("📱 Number (255xxxxxxxxx): ",async n=>{
    ...
    if(sessions.has(n)||activeUsers.has(n)){
     console.log(`ℹ️ ${n} already connected`);
     // Show pairing code if exists
     if(pairingCodes.has(n)){
      const d=pairingCodes.get(n);
      console.log(`🔑 Code: ${d.code} (${Date.now()>d.expires?'EXPIRED':'Valid'})`);
     }
     askForUser();
     return;
    }
  ...
  if(c==='4'){
   rl.question("📱 Number to remove: ",async n=>{
    ...
    else if(activeUsers.has(n)){activeUsers.delete(n);pairingCodes.delete(n);console.log(`✅ ${n} removed from active users`);}
