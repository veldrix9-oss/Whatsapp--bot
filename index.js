...
try{switch(c){case"menu":await menu(w,j);break;case"ping":await w.sendMessage(j,{text:"🏓 Pong! Online ✅"});break;case"owner":await w.sendMessage(j,{text:`👑 Owner: ${C.owner}\nPhone: ${C.botNumber}\nStatus: Online 🌟`});break;case"status":await status(w,j);break;case"alive":await w.sendMessage(j,{text:`🤖 ${C.name}\n✅ Alive!\n📱 ${moment().format("YYYY-MM-DD HH:mm:ss")}\n💖 ${C.owner}`});break;case"runtime":const r=process.uptime();await w.sendMessage(j,{text:`⏱️ ${Math.floor(r/86400)}d ${Math.floor((r%86400)/3600)}h ${Math.floor((r%3600)/60)}m ${Math.floor(r%60)}s`});break;case"repo":await w.sendMessage(j,{text:"📦 https://github.com/veldrix/whatsapp-bot"});break;case"public":C.mode="public";await w.sendMessage(j,{text:"✅ PUBLIC for everyone!"});break;case"self":C.mode="self";await w.sendMessage(j,{text:"✅ SELF mode"});break;case"tagall":if(g)await tagAll(w,j);else await w.sendMessage(j,{text:"❌ Groups only"});break;case"online":case"listonline":if(g)await onlineMembers(w,j);else await w.sendMessage(j,{text:"❌ Groups only"});break;case"admins":case"listadmins":if(g)await listAdmins(w,j);else await w.sendMessage(j,{text:"❌ Groups only"});break;case"tagadmin":if(g)await tagAdmin(w,j);else await w.sendMessage(j,{text:"❌ Groups only"});break;case"promote":if(g&&args.length>0)await promote(w,j,args[0]);else await w.sendMessage(j,{text:"❌ .promote @user"});break;case"demote":if(g&&args.length>0)await demote(w,j,args[0]);else await w.sendMessage(j,{text:"❌ .demote @user"});break;case"kick":case"remove":if(g&&args.length>0)await kick(w,j,args[0]);else await w.sendMessage(j,{text:"❌ .kick @user"});break;case"add":if(g&&args.length>0)await add(w,j,args[0]);else await w.sendMessage(j,{text:"❌ .add @user"});break;case"leave":if(g){await w.groupLeave(j);await w.sendMessage(j,{text:"👋 Left!"});}else await w.sendMessage(j,{text:"❌ Groups only"});break;case"groupinfo":if(g)await groupInfo(w,j);else await w.sendMessage(j,{text:"❌ Groups only"});break;case"welcome":C.welcome=!C.welcome;await w.sendMessage(j,{text:`✅ Welcome ${C.welcome?"enabled":"disabled"}`});break;case"hidetag":if(g)await hidetag(w,j);else await w.sendMessage(j,{text:"❌ Groups only"});break;case"react":if(args.length>0){const v=parseFloat(args[0]);if(!isNaN(v)&&v>=0&&v<=100){C.reactChance=v/100;await w.sendMessage(j,{text:`✅ Set to ${C.reactChance*100}%`});}else await w.sendMessage(j,{text:"❌ Use 0-100"});}else await w.sendMessage(j,{text:`📊 ${C.reactChance*100}%`});break;case"autoreact":C.reactChance=C.reactChance>0?0:.3;await w.sendMessage(j,{text:`✅ Auto-react ${C.reactChance>0?"enabled":"disabled"}`});break;case"help":await help(w,j);break;case"info":await w.sendMessage(j,{text:`🤖 ${C.name}\nVer: ${C.ver}\nMode: ${C.mode}\nFeatures: Auto-status, Anti-spam, Anti-ban, Welcome, Group Mgmt, Fun`});break;case"time":await w.sendMessage(j,{text:`⏰ ${moment().format("HH:mm:ss")}`});break;case"date":await w.sendMessage(j,{text:`📅 ${moment().format("dddd, MMMM Do YYYY")}`});break;case"joke":await w.sendMessage(j,{text:joke()});break;case"quote":await w.sendMessage(j,{text:quote()});break;case"fact":await w.sendMessage(j,{text:fact()});break;case"advice":await w.sendMessage(j,{text:advice()});break;case"meme":await w.sendMessage(j,{text:meme()});break;case"truth":await w.sendMessage(j,{text:truth()});break;case"dare":await w.sendMessage(j,{text:dare()});break;default:console.log(`❓ ${c}`);}}catch(e){console.log(`❌ ${e.message}`);}return;}}catch(e){console.log(`❌ ${e.message}`);}});

// GROUP PARTICIPANTS UPDATE - WITH PROFILE ICON AND FANCY FONTS
...
for(const x of a){try{const pp=await w.profilePictureUrl(x,"image").catch(()=>null);
if(o==="add"){let msg=`🎉 *━━━━━━━━━━━━━━━━━━━━━━━* 🎉\n\n`;
msg+=`✨ *WELCOME TO THE GROUP!* ✨\n\n`;
msg+=`👤 *Name:* @${x.split('@')[0]}\n`;
msg+=`📱 *Number:* ${x.split('@')[0]}\n`;
msg+=`📅 *Joined:* ${moment().format("MMMM Do YYYY, h:mm:ss A")}\n`;
if(pp){msg+=`🖼️ *Profile:* ✅ (Picture Available)\n`;}else{msg+=`🖼️ *Profile:* ❌ (No Picture)\n`;}
...
msg+=`🕐 ${moment().format("MMMM Do YYYY, h:mm:ss A")}\n\n`;
msg+=`💫 *━━━━━━━━━━━━━━━━━━━━━━━* 💫`;
await w.sendMessage(p,{text:msg,mentions:[x]});}}catch(e){console.log(`❌ Member update error: ${e.message}`);}}}catch(e){console.log(`❌ Group update error: ${e.message}`);}});
...
async function help(w,j){await w.sendMessage(j,{text:`╔══✦ 📖 COMMAND HELP 📖 ✦══╗\n\n🔹 OWNER:\n.menu .ping .owner .status .alive .runtime .repo .public .self\n\n🔹 GROUP:\n.welcome - Toggle welcome\n.tagall - Tag all\n.online - Online members\n.listonline - List online\n.admins - List admins\n.listadmins - List admins\n.tagadmin - Tag admins\n.promote @user - Make admin\n.demote @user - Remove admin\n.kick @user - Remove member\n.remove @user - Remove member\n.add @user - Add member\n.leave - Bot leaves\n.groupinfo - Group info\n.hidetag - Hidden tag\n\n🔹 TOOL:\n.react [0-100] - Set reaction\n.autoreact - Toggle\n.help - This help\n.info - Bot info\n.time - Current time\n.date - Current date\n\n🔹 FUN:\n.joke .quote .fact .advice .meme .truth .dare\n\n✨ AUTO: Auto-status, Anti-spam, Anti-ban, Auto-reconnect, Welcome with Profile\n💖 ${C.ver} 👨‍💻 ${C.owner}`});}

async function onlineMembers(w,j){try{const m=await w.groupMetadata(j);const p=m.participants;let on=[],off=[];for(const x of p){try{const pr=await w.presenceSubscribe(x.id);if(pr&&pr.lastKnownPresence==="available")on.push(x.id);else off.push(x.id);}catch(e){off.push(x.id);}}let t="👥 ONLINE MEMBERS\n\n";if(on.length>0){t+="🟢 Online:\n";for(const o of on)t+=`• @${o.split('@')[0]}\n`;}else t+="🟢 No one online\n";t+=`\n⚫ Offline: ${off.length}\n📊 Total: ${p.length}`;await w.sendMessage(j,{text:t,mentions:on});}catch(e){}}
