...
case"kick":case"remove":if(g&&args.length>0)await kick(w,j,args[0]);else await w.sendMessage(j,{text:"❌ .kick/@remove @user"});break;
...
case"songfrommood":if(args.length>0){const songs={happy:["Happy - Pharrell Williams","Walking on Sunshine","Don't Stop Believin'"],sad:["Someone Like You - Adele","Fix You - Coldplay","Yesterday"],angry:["Break Stuff - Limp Bizkit","Killing in the Name","Bulls on Parade"],romantic:["Perfect - Ed Sheeran","All of Me - John Legend","Thinking Out Loud"]};const list=songs[args[0]]||songs.happy;await w.sendMessage(j,{text:`🎵 *SONG FROM MOOD*\n\n😊 Mood: ${args[0].toUpperCase()}\n🎶 ${list[Math.floor(Math.random()*list.length)]}\n\n🎧 Enjoy!`});}break;
...
default:console.log(`❓ ${c}`);}}catch(e){console.log(`❌ Command error: ${e.message}`);}}}catch(e){console.log(`❌ Message error: ${e.message}`);}});

// GROUP PARTICIPANTS UPDATE - PROFILE IMAGE AS ACTUAL IMAGE
...
// FIXED ONLINE MEMBERS - REAL ONLINE PEOPLE
...
async function hidetag(w,j){try{const m=await w.groupMetadata(j);const p=m.participants;let mts=[];for(const x of p)mts.push(x.id);await w.sendMes
